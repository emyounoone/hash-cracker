...
def crack(hashvalue):
    ...
    if len(hashvalue) == 32:
        if not file:
            print ('%s Hash fonksiyonu : MD5' % info)
        for api in md5:
            r = api(hashvalue, 'md5')
            if r:
                return r
        ...
        if not file:
            print ('%s Hash fonksiyonu : SHA1' % info)
        for api in sha1:
        ...
        if not file:
            print ('%s Hash fonksiyonu : SHA-256' % info)
        for api in sha256:
        ...
        if not file:
            print ('%s Hash fonksiyonu : SHA-384' % info)
        for api in sha384:
        ...
        if not file:
            print ('%s Hash fonksiyonu : SHA-512' % info)
        for api in sha512:
        ...
        if not file:
            print ('%s Hash tipi desteklenmiyor.' % bad)
            quit()
        else:
            return False
...
def grepper(directory):
    ...
    print ('%s Sonuçlar buraya kaydedildi: %s.txt' % (info, directory.split('/')[-1]))
...
def single(args):
    ...
    if result:
        ...
        print ('%s Hash herhangi bir databasede bulunamadı :(' % bad)
...
if directory:
    ...
    print ('%s Sonuçlar kayıt edildi-%s' % (info, file.split('/')[-1]))
